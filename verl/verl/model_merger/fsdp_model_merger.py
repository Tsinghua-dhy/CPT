...
class FSDPModelMerger(BaseModelMerger):
    ...
    def _count_actual_shards(self, world_size: int) -> int:
        """Count the actual number of model shard files on disk."""
        count = 0
        while True:
            model_path = Path(self.config.local_dir) / f"model_world_size_{world_size}_rank_{count}.pt"
    ...
    def merge_and_save(self):
        ...
        # Sanity check: if actual shard files on disk are fewer than total_shards,
        # this likely means DDP+FSDP where only one DDP replica was saved.
        actual_shards = self._count_actual_shards(world_size)
        if actual_shards < total_shards:
            print(f"⚠️  Warning: expected {total_shards} shards but only found {actual_shards} files on disk. "
                  f"This is likely a DDP+FSDP checkpoint where only one replica was saved. "
                  f"Using actual file count ({actual_shards}) instead.")
            total_shards = actual_shards
            mesh_shape = (actual_shards,)

...
class SFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: str | ListConfig, tokenizer, config):
        ...
        # Debug: print first N samples' input/output for verification
        self.enable_token_debug = config.get("enable_token_debug", False)
        self.debug_num_samples = config.get("debug_num_samples", 3)
        self._debug_count = 0
    ...
    def __getitem__(self, item):
        ...
        if self.use_chat_template:
            ...
            # For base models, prompt and response are concatenated as plain text
        ...
        # Debug: print detailed input/output for first N samples
        if self.enable_token_debug and self._debug_count < self.debug_num_samples:
            self._debug_count += 1
            print("=" * 80)
            print(f"[SFT DEBUG] Sample {item} (debug {self._debug_count}/{self.debug_num_samples}):")
            print(f"  use_chat_template: {self.use_chat_template}")
            print(f"  prompt_length: {prompt_length} tokens")
            print(f"  response_length: {response_length} tokens")
            print(f"  total_length: {prompt_length + response_length} tokens")
            print(f"  max_length: {self.max_length}")
            print("-" * 40)
            print(f"  [PROMPT] (first 500 chars):")
            print(f"    {prompt_chat_str[:500]}")
            print(f"  [PROMPT] (last 200 chars):")
            print(f"    ...{prompt_chat_str[-200:]}")
            print("-" * 40)
            print(f"  [RESPONSE] (first 500 chars):")
            print(f"    {response_chat_str[:500]}")
            print(f"  [RESPONSE] (last 200 chars):")
            print(f"    ...{response_chat_str[-200:]}")
            print("-" * 40)
            print(f"  [PROMPT TOKEN IDS] (first 20): {prompt_ids[:20].tolist()}")
            print(f"  [PROMPT TOKEN IDS] (last 20):  {prompt_ids[-20:].tolist()}")
            print(f"  [RESPONSE TOKEN IDS] (first 20): {response_ids[:20].tolist()}")
            print(f"  [RESPONSE TOKEN IDS] (last 20):  {response_ids[-20:].tolist()}")
            print("-" * 40)
            ...
            print(f"  [DECODED PROMPT] (first 300 chars):")
            print(f"    {decoded_prompt[:300]}")
            print(f"  [DECODED RESPONSE] (first 300 chars):")
            print(f"    {decoded_response[:300]}")
            print("=" * 80)

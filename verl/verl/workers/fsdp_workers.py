...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        # Debug: print first batch training information
        if not hasattr(self, '_train_debug_counter'):
            self._train_debug_counter = 0
        
        if self._train_debug_counter == 0 and torch.distributed.get_rank() == 0:
            print(f"\n{'='*80}")
            print(f"[TRAINING DEBUG] First batch compute_log_prob:")
            print(f"Batch size: {data.batch.batch_size}")
            
            # Show first 2 samples
            for i in range(min(2, data.batch.batch_size[0])):
                input_ids = data.batch["input_ids"][i]
                attention_mask = data.batch["attention_mask"][i]
                
                # Find the actual sequence (non-padding part)
                non_pad_mask = attention_mask.bool()
                actual_seq = input_ids[non_pad_mask]
                
                print(f"\n  Sample {i}:")
                print(f"    Total length: {input_ids.shape[0]} tokens")
                print(f"    Actual length (non-padding): {actual_seq.shape[0]} tokens")
                print(f"    First 30 token IDs: {actual_seq[:30].tolist()}")
                print(f"    Last 30 token IDs: {actual_seq[-30:].tolist()}")
                
                # Decode the sequence
                try:
                    if hasattr(self, 'tokenizer'):
                        decoded = self.tokenizer.decode(actual_seq, skip_special_tokens=False)
                        print(f"    Decoded text (first 300 chars):\n{decoded[:300]}")
                        print(f"    Decoded text (last 200 chars):\n{decoded[-200:]}")
                except Exception as e:
                    print(f"    Failed to decode: {e}")
                
                # Show prompt vs response split
                if "prompts" in data.batch and "responses" in data.batch:
                    prompt_ids = data.batch["prompts"][i]
                    response_ids = data.batch["responses"][i]
                    prompt_mask = attention_mask[:prompt_ids.shape[0]]
                    actual_prompt = prompt_ids[prompt_mask[:prompt_ids.shape[0]].bool()]
                    
                    print(f"    Prompt length: {actual_prompt.shape[0]} tokens")
                    print(f"    Response length: {response_ids.shape[0]} tokens")
            
            print(f"{'='*80}\n")
            print(flush=True)
            self._train_debug_counter += 1
